import {
  ...
  const ProfileScreen = ({ navigation, navigation: { goBack }, route }) => {
    ...
    // console.log(userInfo?.photos?.cover);
    ...
    const updatePhotos = async (fileName, type) => {
      try {
        ...
        try {
          ...
          console.log(`Error retrieving URLs for image : ${error.message}`);
        ...
        if (type == "avatar") {
          return (resizedPhotos = [resized1, resized2, resized3, resized4]);
        } else {
          return (resizedPhotos = [resized1, resized2, resized3]);
        }
      } catch (error) {
        console.log(error);
      }
    };
    const asdas = new Date();
  
    const uploadPhotos = async (fileName, type) => {
      try {
        ...
        const blob = await fetch(type == "avatar" ? avatarUri : coverUri).then(
    ...
    const saveChanges = async () => {
      ...
      if (avatarUri) {
        await uploadPhotos(fileName, "avatar");
      }
      if (coverUri) {
        await uploadPhotos(fileName, "cover");
      }
      ...
      if (avatarUri) {
        ...
        updates = {
          ...userInfo,
          photos: { ...userInfo?.photos, avatar: resizedAvatar },
        };
      }
      if (coverUri) {
        ...
        updates = {
          ...userInfo,
          photos: { ...userInfo?.photos, cover: resizedCover },
        };
      }
          ...
          <ScrollView>
            ...
            <View style={styles.modalContainer}>
              ...
              <TextInput
                ...
                onChangeText={(text) =>
                  setUserInfo({ ...userInfo, phone: { number: Number(text) } })
                }
              />
              <Text
                style={{
