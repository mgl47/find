export const trendingEvents = [
  {
    id: 22,
    valid: true,
    tickets: [
      {
        description: "1 bilhete",
        id: 1,
        amount: 2000,
        type: "Normal",
      },
      {
        id: 2,
        amount: 4000,
        description: "VIP",
        type: "VIP",
      },
    ],
    artistas: [
      {
        id: 6,
        displayName: "Cesária Évora",
        avatar: "https://via.placeholder.com/150",
      },
    ],
    going: 50,
    title: "Concerto Tributo a Cesária Évora",
    date: "Sáb, 15 Ago - 18h00",
    description:
      "Junte-se a nós para um concerto tributo dedicado à lendária cantora cabo-verdiana, Cesária Évora. Experimente uma noite repleta de suas melodias atemporais e voz soulful, enquanto celebramos seu legado e contribuições para a música.",
    city: "Mindelo",
    videos: {
      id: 2,
      uri: "https://via.placeholder.com/300",
    },
    photos: [
      {
        id: 2,
        uri: "https://static.wixstatic.com/media/4c3768_02d48342c7864dbe9ae9cf30d1569509~mv2.jpg/v1/fill/w_595,h_842,al_c/4c3768_02d48342c7864dbe9ae9cf30d1569509~mv2.jpg",
      },
    ],
    promotor: {
      id: 2,
      telefone1: 9876543,
      displayName: "Eventos Cabo Verde",
      avatar: "https://via.placeholder.com/150",
    },
    venue: {
      id: 2,
      displayName: "Praça Nova",
      city: "Mindelo",
      lat: 16.89006,
      long: -24.98767,
      uri: "https://via.placeholder.com/600",
    },
    category: "Música",
  },
  {
    id: 1,
    valid: true,
    tickets: [
      {
        description: "Um bilhete",
        id: 1,
        amount: 3000,
        type: "Promo",
      },
      {
        id: 2,
        amount: 5000,
        description: "Um bilhete fila da frente e uma garrafa das fas sfa ",
        type: "Normal",
      },
      {
        id: 3,
        amount: 10000,
        description: "Um bilhete Vip e duas garrafas",
        type: "Vip",
      },
    ],
    artists: [
      {
        id: 1,
        displayName: "Djodje",
        avatar:
          "https://pbs.twimg.com/profile_images/1288603170957152258/_IhZqJmE_400x400.jpg",
      },
      {
        id: 2,
        displayName: "Danni Gato",
        avatar:
          "https://images.xceed.me/artists/covers/danni-gato-artist-xceed-cover-e08e.jpg",
      },
      {
        id: 3,
        displayName: "Mc Prego Prego",
        avatar:
          "https://locomc.com/pt/wp-content/uploads/sites/2/2023/06/PregoPrego-carousel-Image.jpg",
      },
      {
        id: 4,
        displayName: "Neyna",
        avatar:
          "https://universalmusic.pt/wp-content/uploads/sites/55/2022/04/Neyna.jpg",
      },
      {
        id: 5,
        displayName: "Mc Tranka Fulha",
        avatar:
          "https://1.bp.blogspot.com/-lOah8Alvdy4/XzbI483ZTNI/AAAAAAAAMGw/kmNbP-E09h8_5PnkEf0m7GjFGdGqDq_DACLcBGAsYHQ/w1200-h630-p-k-no-nu/mc%2Btranka%2Bfulha%2Bportugues%2Bdfd.jpg",
      },
    ],
    going: 74,
    title: "Rolling Loud Cv: Edição 2024",
    date: "Qua, 06-08 Jul - 15h00",
    description:
      "Prepare-se para experimentar a fusão definitiva de música, cultura e emoção no Rolling Loud CV 2024! Junte-se a nós para um fim de semana inesquecível de performances sem parar dos artistas mais quentes do hip-hop, rap e além. Dos cabeças de cartaz icônicos aos talentos emergentes, nossa programação promete eletrizar cada momento. Mergulhe nos ritmos pulsantes, na atmosfera vibrante e na energia incomparável enquanto transformamos Cabo Verde no epicentro da cena musical global. Garanta seu lugar agora e faça parte da história sendo feita no Rolling Loud CV 2024!",
    city: "Praia",
    videos: [
      {
        id: 1,
        uri: "https://firebasestorage.googleapis.com/v0/b/dtudo2-fac2e.appspot.com/o/media%2Frolling.mp4?alt=media&token=8de1dd1c-b324-4522-94f8-ad4207e05c3e",
      },
    ],
    photos: [
      {
        id: 1,
        uri: "https://d1mnxluw9mpf9w.cloudfront.net/media/13277/Rolling-loud-logo.jpg",
      },
      {
        id: 2,
        uri: "https://d17t27i218htgr.cloudfront.net/rails/active_storage/representations/proxy/eyJfcmFpbHMiOnsibWVzc2FnZSI6IkJBaEpJaWsxTW1GaVpqVmlOeTA0WVdVeExUUm1NakF0WVRNeFlpMDRZVGhqTTJRellUQm1PR1FHT2daRlZBPT0iLCJleHAiOm51bGwsInB1ciI6ImJsb2JfaWQifX0=--a8233269a8be523b47bc647747c984447721382e/eyJfcmFpbHMiOnsibWVzc2FnZSI6IkJBaDdDRG9MWm05eWJXRjBTU0lJYW5CbkJqb0dSVlE2RkhKbGMybDZaVjkwYjE5c2FXMXBkRnNIYVFLQUJ6QTZDbk5oZG1WeWV3YzZDbk4wY21sd1ZEb01jWFZoYkdsMGVXbGEiLCJleHAiOm51bGwsInB1ciI6InZhcmlhdGlvbiJ9fQ==--1cca388767d511d8884c9fa9b0eb35c8aea0be6b/RLPortugal2023FINALLineupFullRes.jpg",
      },
      {
        id: 3,
        uri: "https://res.cloudinary.com/tjacob/image/upload/f_jpg,q_90,w_1250/rolling_loud_2_94c503e54c.jpg",
      },
      {
        id: 4,
        uri: "https://d1mnxluw9mpf9w.cloudfront.net/media/5093/rolling-loud.jpg",
      },
    ],
    promoter: {
      id: 1,
      phone1: 9123456,
      displayName: "Sigue Sabura",
      avatar:
        "https://yt3.googleusercontent.com/ytc/AIf8zZR_Clt727dpdMZ_o5Hs1I5D-FVVtwcG279N_Id-=s900-c-k-c0x00ffffff-no-rj",
    },
    venue: {
      id: 1,
      displayName: "Kebra Cabana",
      city: "Praia",
      lat: 14.904463,
      long: -23.517431,
      uri: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRk61TxhkRQ7IoMiZY_VtxS6SsAian0TbCqjvRDrLpURA&s",
    },
    Category: "Music",
  },
  {
    id: 123,

    title: "Prendi cuzinha ku Tanxinha - Bolachas  da fds  afa ",
    date: "Qua, 25 Mai - 16h00",
    venue: {
      id: 1,
      displayName: "Parque 5 de Julho",
      uri: "https://scontent.fopo3-1.fna.fbcdn.net/v/t39.30808-6/277743931_5688317584517425_7404246851532435527_n.jpg?_nc_cat=101&ccb=1-7&_nc_sid=efb6e6&_nc_eui2=AeFGFWlGhsBPmPONu3bpvGXlxpbOmYh47GfGls6ZiHjsZxGoeLMmWAVQG4b3UZxX3_hUt7KcemVPYfNEf-vx5y0q&_nc_ohc=sTcHo3xDP8UAX9KWrYs&_nc_ht=scontent.fopo3-1.fna&oh=00_AfDHNQvMtUnyuQb03LTtA5Ky7F2P1wN9ezkbp-GxZsI3NA&oe=65E95566",

      address: {
        city: "Praia",
        zone: "Achadda Santo António",
        lat: 14.904463,
        long: -23.517431,
      },
    },
    photos: [
      {
        id: 1,
        uri: "https://images.unsplash.com/photo-1558961363-fa8fdf82db35?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=765&q=80",
      },
      {
        id: 2,
        uri: "https://d1mnxluw9mpf9w.cloudfront.net/media/13277/Rolling-loud-logo.jpg",
      },
      {
        id: 3,
        uri: "https://d1mnxluw9mpf9w.cloudfront.net/media/13277/Rolling-loud-logo.jpg",
      },
      {
        id: 4,
        uri: "https://d1mnxluw9mpf9w.cloudfront.net/media/13277/Rolling-loud-logo.jpg",
      },
    ],
    interest: "21 people are going",
    onPress: "Cookies",
  },

  {
    id: 3,

    title: "Starladder Major 2022",
    date: "Fri, 27-29 Mai - 4:00 pm",
    city: "Berlin",
    photos: [
      {
        id: 1,
        uri: "https://editors.dexerto.com/wp-content/uploads/thumbnails/_thumbnailLarge/StarLadder-Berlin-Major-2019-Round-1-matches-revealed-full-schedule.jpg",
      },
      {
        id: 2,
        uri: "https://d1mnxluw9mpf9w.cloudfront.net/media/13277/Rolling-loud-logo.jpg",
      },
      {
        id: 3,
        uri: "https://d1mnxluw9mpf9w.cloudfront.net/media/13277/Rolling-loud-logo.jpg",
      },
      {
        id: 4,
        uri: "https://d1mnxluw9mpf9w.cloudfront.net/media/13277/Rolling-loud-logo.jpg",
      },
    ],
    Category: "Esports",
    onPress: "Cs",
  },

  {
    id: 4,

    title: "Color Content",
    date: "Qui, 14 Jul - 10h00",
    city: "Wroclaw",
    photos: [
      {
        id: 1,
        uri: "https://images.unsplash.com/photo-1456086272160-b28b0645b729?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1032&q=80",
      },
      {
        id: 2,
        uri: "https://d1mnxluw9mpf9w.cloudfront.net/media/13277/Rolling-loud-logo.jpg",
      },
      {
        id: 3,
        uri: "https://d1mnxluw9mpf9w.cloudfront.net/media/13277/Rolling-loud-logo.jpg",
      },
      {
        id: 4,
        uri: "https://d1mnxluw9mpf9w.cloudfront.net/media/13277/Rolling-loud-logo.jpg",
      },
    ],
    Category: "Art",
    onPress: "Color",
  },
];
export const recommendedEvents = [
  {
    id: 1,
    title: "Sun7ven",
    date: "Sex, 27 Mai - 16h30",
    city: "Praia",
    venue: {
      id: 1,
      displayName: "Linha d'Água",
      uri: "https://scontent.fopo3-1.fna.fbcdn.net/v/t39.30808-6/277743931_5688317584517425_7404246851532435527_n.jpg?_nc_cat=101&ccb=1-7&_nc_sid=efb6e6&_nc_eui2=AeFGFWlGhsBPmPONu3bpvGXlxpbOmYh47GfGls6ZiHjsZxGoeLMmWAVQG4b3UZxX3_hUt7KcemVPYfNEf-vx5y0q&_nc_ohc=sTcHo3xDP8UAX9KWrYs&_nc_ht=scontent.fopo3-1.fna&oh=00_AfDHNQvMtUnyuQb03LTtA5Ky7F2P1wN9ezkbp-GxZsI3NA&oe=65E95566",

      address: {
        city: "Praia",
        zone: "Prainha",
        lat: 14.904463,
        long: -23.517431,
      },
    },

    photos: [
      {
        id: 1,
        uri: "https://images.unsplash.com/photo-1635861496491-4fd2cb740dfa?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=704&q=80",
      },
      {
        id: 2,
        uri: "https://d1mnxluw9mpf9w.cloudfront.net/media/13277/Rolling-loud-logo.jpg",
      },
      {
        id: 3,
        uri: "https://d1mnxluw9mpf9w.cloudfront.net/media/13277/Rolling-loud-logo.jpg",
      },
      {
        id: 4,
        uri: "https://d1mnxluw9mpf9w.cloudfront.net/media/13277/Rolling-loud-logo.jpg",
      },
    ],
    interest: "36 people are going",
    onPress: "Sun7ven",
  },
  {
    id: 2,

    title: "Espaço Y",
    date: "Dom, 05 June - 13h00",
    venue: {
      id: 1,
      displayName: "Auditório Nacional",
      uri: "https://scontent.fopo3-1.fna.fbcdn.net/v/t39.30808-6/277743931_5688317584517425_7404246851532435527_n.jpg?_nc_cat=101&ccb=1-7&_nc_sid=efb6e6&_nc_eui2=AeFGFWlGhsBPmPONu3bpvGXlxpbOmYh47GfGls6ZiHjsZxGoeLMmWAVQG4b3UZxX3_hUt7KcemVPYfNEf-vx5y0q&_nc_ohc=sTcHo3xDP8UAX9KWrYs&_nc_ht=scontent.fopo3-1.fna&oh=00_AfDHNQvMtUnyuQb03LTtA5Ky7F2P1wN9ezkbp-GxZsI3NA&oe=65E95566",

      address: {
        city: "Praia",
        zone: "Achadda Santo António",
        lat: 14.904463,
        long: -23.517431,
      },
    },
    photos: [
      {
        id: 1,
        uri: "https://jornaleconomico.sapo.pt/wp-content/uploads/2021/07/palco.jpg",
      },
      {
        id: 2,
        uri: "https://d1mnxluw9mpf9w.cloudfront.net/media/13277/Rolling-loud-logo.jpg",
      },
      {
        id: 3,
        uri: "https://d1mnxluw9mpf9w.cloudfront.net/media/13277/Rolling-loud-logo.jpg",
      },
      {
        id: 4,
        uri: "https://d1mnxluw9mpf9w.cloudfront.net/media/13277/Rolling-loud-logo.jpg",
      },
    ],
    interest: "74 people are going",
    onPress: "SpaceY",
  },
  {
    id: 3,

    title: "Prendi cuzinha ku Tanxinha - Bolachas",
    date: "Qua, 25 Mai - 16h00",
    venue: {
      id: 1,
      displayName: "Parque 5 de Julho",
      uri: "https://scontent.fopo3-1.fna.fbcdn.net/v/t39.30808-6/277743931_5688317584517425_7404246851532435527_n.jpg?_nc_cat=101&ccb=1-7&_nc_sid=efb6e6&_nc_eui2=AeFGFWlGhsBPmPONu3bpvGXlxpbOmYh47GfGls6ZiHjsZxGoeLMmWAVQG4b3UZxX3_hUt7KcemVPYfNEf-vx5y0q&_nc_ohc=sTcHo3xDP8UAX9KWrYs&_nc_ht=scontent.fopo3-1.fna&oh=00_AfDHNQvMtUnyuQb03LTtA5Ky7F2P1wN9ezkbp-GxZsI3NA&oe=65E95566",

      address: {
        city: "Praia",
        zone: "Achadda Santo António",
        lat: 14.904463,
        long: -23.517431,
      },
    },
    photos: [
      {
        id: 1,
        uri: "https://images.unsplash.com/photo-1558961363-fa8fdf82db35?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=765&q=80",
      },
      {
        id: 2,
        uri: "https://d1mnxluw9mpf9w.cloudfront.net/media/13277/Rolling-loud-logo.jpg",
      },
      {
        id: 3,
        uri: "https://d1mnxluw9mpf9w.cloudfront.net/media/13277/Rolling-loud-logo.jpg",
      },
      {
        id: 4,
        uri: "https://d1mnxluw9mpf9w.cloudfront.net/media/13277/Rolling-loud-logo.jpg",
      },
    ],
    interest: "21 people are going",
    onPress: "Cookies",
  },
];
export const newEvents = [
  {
    id: 2,
    valid: true,
    tickets: [
      {
        description: "1 bilhete",
        id: 1,
        amount: 2000,
        type: "Normal",
      },
      {
        id: 2,
        amount: 4000,
        description: "VIP",
        type: "VIP",
      },
    ],
    artistas: [
      {
        id: 6,
        displayName: "Cesária Évora",
        avatar: "https://via.placeholder.com/150",
      },
    ],
    going: 50,
    title: "Concerto Tributo a Cesária Évora",
    date: "Sáb, 15 Ago - 18h00",
    description:
      "Junte-se a nós para um concerto tributo dedicado à lendária cantora cabo-verdiana, Cesária Évora. Experimente uma noite repleta de suas melodias atemporais e voz soulful, enquanto celebramos seu legado e contribuições para a música.",
    city: "Mindelo",
    videos: {
      id: 2,
      uri: "https://via.placeholder.com/300",
    },
    photos: [
      {
        id: 2,
        uri: "https://static.wixstatic.com/media/4c3768_02d48342c7864dbe9ae9cf30d1569509~mv2.jpg/v1/fill/w_595,h_842,al_c/4c3768_02d48342c7864dbe9ae9cf30d1569509~mv2.jpg",
      },
    ],
    promotor: {
      id: 2,
      telefone1: 9876543,
      displayName: "Eventos Cabo Verde",
      avatar: "https://via.placeholder.com/150",
    },
    venue: {
      id: 2,
      displayName: "Praça Nova",
      city: "Mindelo",
      lat: 16.89006,
      long: -24.98767,
      uri: "https://via.placeholder.com/600",
    },
    category: "Música",
  },
  // Adicione mais eventos aqui

  {
    id: 3,
    valid: true,
    tickets: [
      {
        description: "1 bilhete",
        id: 1,
        amount: 2500,
        type: "Normal",
      },
      {
        id: 2,
        amount: 4500,
        description: "VIP com brindes exclusivos",
        type: "VIP",
      },
    ],
    artistas: [
      {
        id: 7,
        displayName: "Mayra Andrade",
        avatar: "https://via.placeholder.com/150",
      },
    ],
    going: 60,
    title: "Noite de Fado com Mayra Andrade",
    date: "Sex, 25 Set - 20h00",
    description:
      "Desfrute de uma noite emocionante de fado com a talentosa cantora cabo-verdiana Mayra Andrade. Deixe-se envolver pela atmosfera única do fado enquanto Mayra nos leva em uma jornada musical emocionante.",
    city: "Assomada",
    videos: {
      id: 3,
      uri: "https://via.placeholder.com/300",
    },
    photos: [
      {
        id: 3,
        uri: "https://www.sonsemtransito.com/news/mayra-andrade-368.jpg",
      },
    ],
    promotor: {
      id: 3,
      telefone1: 9876543,
      displayName: "Associação Cultural de Cabo Verde",
      avatar: "https://via.placeholder.com/150",
    },
    venue: {
      id: 3,
      displayName: "Centro Cultural de Assomada",
      city: "Assomada",
      lat: 15.09924,
      long: -23.68585,
      uri: "https://via.placeholder.com/600",
    },
    category: "Música",
  },
  {
    id: 4,
    valid: true,
    tickets: [
      {
        description: "1 bilhete",
        id: 1,
        amount: 1500,
        type: "Normal",
      },
      {
        id: 2,
        amount: 3000,
        description: "VIP com acesso aos bastidores",
        type: "VIP",
      },
    ],
    artistas: [
      {
        id: 8,
        displayName: "Elida Almeida",
        avatar: "https://via.placeholder.com/150",
      },
    ],
    going: 40,
    title: "Show Acústico de Elida Almeida",
    date: "Sáb, 10 Out - 19h30",
    description:
      "Desfrute de uma noite intimista com a aclamada cantora cabo-verdiana Elida Almeida. Este show acústico proporcionará uma experiência única, onde você poderá apreciar a voz e as músicas de Elida em um ambiente tranquilo e envolvente.",
    city: "Mindelo",
    videos: {
      id: 4,
      uri: "https://via.placeholder.com/300",
    },
    photos: [
      {
        id: 4,
        uri: "https://images.impresa.pt/expresso/2023-03-24-elida-almeida.jpg-1a0424c3",
      },
    ],
    promotor: {
      id: 4,
      telefone1: 9876543,
      displayName: "Associação de Promoção Cultural",
      avatar: "https://via.placeholder.com/150",
    },
    venue: {
      id: 4,
      displayName: "Centro Cultural do Mindelo",
      city: "Mindelo",
      lat: 16.89006,
      long: -24.98767,
      uri: "https://via.placeholder.com/600",
    },
    category: "Música",
  },
  {
    id: 5,
    valid: true,
    tickets: [
      {
        description: "1 bilhete",
        id: 1,
        amount: 1800,
        type: "Normal",
      },
      {
        id: 2,
        amount: 3500,
        description: "VIP com meet & greet",
        type: "VIP",
      },
    ],
    artistas: [
      {
        id: 9,
        displayName: "Tito Paris",
        avatar: "https://via.placeholder.com/150",
      },
    ],
    going: 55,
    title: "Noite de Samba com Tito Paris",
    date: "Sex, 01 Nov - 21h00",
    description:
      "Prepare-se para uma noite cheia de ritmo e energia com o renomado músico cabo-verdiano Tito Paris. Nesta noite especial, Tito trará sua paixão pelo samba para o palco, proporcionando uma experiência musical única que você não vai querer perder!",
    city: "São Vicente",
    videos: {
      id: 5,
      uri: "https://via.placeholder.com/300",
    },
    photos: [
      {
        id: 5,
        uri: "https://www.agendalx.pt/content/uploads/2022/05/Tito-Paris-e1653468883605.png",
      },
    ],
    promotor: {
      id: 5,
      telefone1: 9876543,
      displayName: "Promoções Música e Arte",
      avatar: "https://via.placeholder.com/150",
    },
    venue: {
      id: 5,
      displayName: "Estádio Adérito Sena",
      city: "Mindelo",
      lat: 16.89721,
      long: -24.08795,
      uri: "https://via.placeholder.com/600",
    },
    category: "Música",
  },
  // Adicione mais eventos aqui
];
